...
```bash
docker run --name acdh-repo -p 80:80 -e CFG_BRANCH=arche -d zozlak/acdh-repo
```
...
By default (if you don't use any of above-mentioned options) the branch `master` of the https://github.com/zozlak/acdh-repo-docker-config.git repository is used.
...
The easiest way is to start by forking the https://github.com/zozlak/acdh-repo-docker-config repository. See the repository README for detailed instructions.
...
```bash
git clone https://github.com/zozlak/acdh-repo-docker-config.git shares/config && cd shares/config && git checkout arche
```
